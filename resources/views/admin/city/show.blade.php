<x-default-layout>
    ...
    <div class="row mt-10">
        <div class="12">
            <form method="post" action="{{ route('city.update' , [$data["city"]]) }}">
                @csrf
                <div class="row">
                    <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                        <div class="mb-10">
                            <label for="name" class="required form-label">نام شهر</label>
                            <input id="name" type="text" name="name" class="form-control" value="{{ $data["city"]->name }}" placeholder="شهر">
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                        <div class="mb-10">
                            ...
                            <select id="province" name="province_id" class="form-select">
                                ...
                                @foreach($data["provinces"] as $province)
                                    <option value="{{ $province->id }}" @if($data["city"]->province_id == $province->id) selected @endif>{{ $province->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                        <div class="mb-10">
                            ...
                            <select id="status" name="status" class="form-select">
                                <option value="1" @if($data["city"]->status) selected @endif>فعال</option>
                                <option value="0" @if(!$data["city"]->status) selected @endif>غیر فعال</option>
                            </select>
                        </div>
                    </div>
